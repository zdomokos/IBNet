...
public class XmlHelper
{
    ...
    private static List<AdvisorGroup> GetGroupsList(XmlDocument xmlDocument)
    {
        ...
        {
            AdvisorGroup advisorGroup = new AdvisorGroup(groupsList.Item(i).ChildNodes[0].InnerText,
                                                         groupsList.Item(i).ChildNodes[2].InnerText);
    ...
    private static List<AllocationProfile> GetProfilesList(XmlDocument xmlDocument)
    {
        ...
        {
            AllocationProfile allocationProfile = new AllocationProfile(profilesList.Item(i).ChildNodes[0].InnerText,
                                                                        int.Parse(profilesList.Item(i).ChildNodes[1]
                                                                           .InnerText));
            XmlNodeList allocationNodes = profilesList[i].ChildNodes[2].ChildNodes;
            for (int j = 0; j < allocationNodes.Count; j++)
            {
                allocationProfile.Allocations.Add(new Allocation(allocationNodes[j].ChildNodes[0].InnerText,
                                                                 double.Parse(allocationNodes[j].ChildNodes[1]
                                                                                 .InnerText)));
            }
