...
public class Reader
{
    ...
    public Contracts.FlexQueryResponse GetByString(string xmlFile, Options options = null)
    {
        if (options != null)
        {
            if (options.UseXmlReader)
            {
                ...
                using (XmlReader reader = XmlReader.Create(xmlFile, settings))
                {
                    var result =
                        Deserializer
                           .Deserialize<Xml.Contracts.QueryResponse.FlexQueryResponse,
                                Contracts.FlexQueryResponse>(reader, out var errorObjects, out string mappingError);
        ...
        using (var stream = sb.GenerateStream(xmlFile))
        {
            var result =
                Deserializer
                   .Deserialize<Xml.Contracts.QueryResponse.FlexQueryResponse,
                        Contracts.FlexQueryResponse>(stream, out var errorObjects, out string mappingError);
            result               = result ?? new Contracts.FlexQueryResponse();
            result.Errors        = errorObjects;
            result.MappingErrors = mappingError;
            foreach (FlexStatement statement in result.FlexStatements.FlexStatement)
    ...
    private async Task<FlexResult> GetStatement(HttpClient client, string token, string referenceCode, string dumpFile)
    {
        var uri =
            new
                Uri($"https://gdcdyn.interactivebrokers.com/Universal/servlet/FlexStatementService.GetStatement?q={referenceCode}&t={token}&v=3");
        ...
        using (var stream = await getStatementResult.Content.ReadAsStreamAsync().ConfigureAwait(false))
        {
            ...
            var queryResponse =
                Deserializer
                   .Deserialize<Xml.Contracts.QueryResponse.FlexQueryResponse,
                        Contracts.FlexQueryResponse>(stream, out var errorObjects, out string mappingErrors);
    ...
    private async Task<FlexResult> SendRequest(HttpClient client, string token, string queryId, string dumpFile)
    {
        var uri =
            new
                Uri($"https://gdcdyn.interactivebrokers.com/Universal/servlet/FlexStatementService.SendRequest?t={token}&q={queryId}&v=3");
        ...
        using (var stream = await sendRequestResult.Content.ReadAsStreamAsync().ConfigureAwait(false))
        {
            ...
            var response =
                Deserializer.Deserialize<XmlFlexStatementResponse, FlexStatementResponse>(stream, out var errorObjects,
                    out string mappingErrors);
