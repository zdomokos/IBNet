...
internal class Logic
{
    ...
    {
        var ocTrades = trades
           .Where(t => t.OpenCloseIndicator.HasValue &&
                       t.OpenCloseIndicator.Value.Matches(x => x.HasFlag(OpenClose.C) && x.HasFlag(OpenClose.O)));
        var remainingTrades = trades.Where(x => !ocTrades.Contains(x)).ToList();

        foreach (var ocTrade in ocTrades)
        {
            var lot = lots.First(x => x.TradeDateTime == ocTrade.TradeDateTime && x.BuySell == ocTrade.BuySell &&
                                      x.Description == ocTrade.Description);
            ...
            copy.Quantity = Math.Sign(copy.Quantity.Value) *
                            (Math.Abs(copy.Quantity.Value) - Math.Abs(ocTrade.Quantity.Value));
